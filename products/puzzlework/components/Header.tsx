...
const Header: React.FC = () => {
  ...
  const navItems = [
    { label: 'Top', id: 'hero' },
    { label: 'Connection', id: 'concept' },
    { label: 'Selfishness', id: 'features' },
    { label: 'The Piece', id: 'colors' },
    { label: 'Byproduct', id: 'spec' },
  ];

  return (
    <>
      <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-sm text-zinc-900' : 'bg-transparent text-current'} pt-4 pb-4`}>
        <div className="container mx-auto px-6 flex justify-between items-center relative">
          ...
          <div className="cursor-pointer z-50 relative" onClick={() => window.location.href = '../../'}>
            <img 
              src="../../assets/logo.png" 
              alt="PUZZLE" 
              className={`h-10 w-auto transition-all duration-300 ${isMenuOpen ? 'filter invert' : (scrolled ? '' : 'filter invert')}`}
            />
          </div>
            ...
            <nav className="hidden md:flex space-x-8">
              {navItems.map((item) => (
                <button
                  ...
                  className={`text-sm font-medium uppercase tracking-wider hover:text-zinc-500 transition-colors ${scrolled ? 'text-zinc-800' : 'text-current'}`}
                >
                  {item.label}
                </button>
              ))}
            ...
            <div className="z-50 relative">
              <MagneticButton>
                <button 
                  ...
                  className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors duration-300 clickable ${isMenuOpen ? 'bg-white text-black' : (scrolled ? 'bg-zinc-100 text-black' : 'bg-white/20 backdrop-blur text-white')}`}
                  aria-label="Menu"
                >
        ...
        {/* Scroll Progress Bar */}
        <div className={`absolute bottom-0 left-0 h-[2px] bg-zinc-400 transition-all duration-75 ease-out ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`} style={{ width: `${progress * 100}%` }} />
      </header>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none opacity-[0.03]">
            <span className="text-[25vw] font-black leading-none select-none tracking-tighter">PUZZLE</span>
        </div>
            ...
            <div className="flex flex-col md:flex-row h-full">
                ...
                <nav className="w-full md:w-2/3 flex flex-col justify-center space-y-2 md:space-y-4">
                    ...
                    >
                        <div className={`flex items-baseline transition-transform duration-500 ease-out ${isMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'}`} style={{ transitionDelay: `${index * 100}ms` }}>
                            <span className="text-sm font-mono text-zinc-500 mr-8 w-8 block">0{index + 1}</span>
                            <span className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight group-hover:italic transition-all duration-300">
                                {item.label}
                            </span>
                            {/* Hover Arrow */}
                            <span className="ml-6 opacity-0 group-hover:opacity-100 transform -translate-x-4 group-hover:translate-x-0 transition-all duration-300 text-3xl md:text-5xl text-zinc-400">
                                &rarr;
                            </span>
                        </div>
                    </button>
                ...
                <div className={`w-full md:w-1/3 flex flex-col justify-center md:pl-12 mt-12 md:mt-0 transition-all duration-700 delay-300 ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    <div className="space-y-8">
                         <div>
                            <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-4">Product</h4>
                        ...
                        <div className="pt-8 border-t border-white/5 space-y-4">
                            ...
                            <a href="../../privacy/" className="block text-sm text-zinc-400 hover:text-white transition-colors">Privacy Policy</a>
                            <a href="../../terms/" className="block text-sm text-zinc-400 hover:text-white transition-colors">Terms of Service</a>
                        </div>
                    </div>
                </div>
            </div>

            <div className={`mt-auto pt-8 border-t border-white/10 flex justify-between items-end text-xs text-zinc-500 uppercase tracking-widest transition-opacity duration-500 delay-500 ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`}>
                <span>Puzzle Inc.</span>
                <span>Puzzle Work Special Page</span>
            </div>
        </div>
      </div>
    </>
